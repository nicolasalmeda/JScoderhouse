$(document).ready(function () {
  ...
  $("#sd").click(() => {
    ...
    $("#egresos").show();
    $("#porcentaje").show();
    $("#presupuesto").fadeOut(7000);
    $("#ingresos").fadeOut(7000);
    $("#egresos").fadeOut(7000);
    $("#porcentaje").fadeOut(7000);
  });
  ...
  $("#bf").click(function () {
    ...
    if (description.value != "" && valor.value !== "" && valor.value !== "0") {
  ...
  $.getJSON("js/monedaCompra.json", function (respuesta, estado) {
    if (estado === "success") {
      let misDatos = respuesta;
      for (const dato of misDatos) {
        $("#moneda_compra").append(` <div class="elemento limpiarEstilos">
          <div class="elemento_description">${dato.description}</div>
          <div class="derecha limpiarEstilos">
            <div class="elemento_valor">${dato.simbolo} ${dato.valor}</div>
  ...
  $.getJSON("js/monedaVenta.json", function (respuesta, estado) {
    if (estado === "success") {
      let misDatos = respuesta;
      for (const dato of misDatos) {
        $("#moneda_venta").append(` <div class="elemento limpiarEstilos">
          <div class="elemento_description">${dato.description}</div>
          <div class="derecha limpiarEstilos">
            <div class="elemento_valor">${dato.simbolo} ${dato.valor}</div>
            </div>
